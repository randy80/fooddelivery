<template>
    ...
    <v-card style="width:450px;" outlined>
        ...
        <v-card-text>
            ...
            <String label="StoreOrderStatus" v-model="value.storeOrderStatus" :editMode="editMode"/>
    ...
    export default {
        ...
        methods: {
            ...
            async orderAccept() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['orderaccept'].href))
            ...
            async orderReject() {
                try {
                    if(!this.offline) {
                        var temp = await axios.put(axios.fixUrl(this.value._links['orderreject'].href))
                        for(var k in temp.data) {
                            this.value[k]=temp.data[k];
                        }
                    }
